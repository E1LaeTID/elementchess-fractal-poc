...
représenter le temps et les transformations de l'interstice ;
produire les signatures actives, passives ou neutres consommées au tour
suivant.
...
```
Pour chaque joueur :
placer un ou plusieurs jetons autorisés, les conserver ou recycler une
parcelle non capturée ;
déplacer une pièce selon les règles d'échecs ;
résoudre un éventuel combat ;
orienter la pièce déplacée ;
verrouiller l'orientation.
À l'interstice :
trois éléments distincts sont tirés ;
les orientations correspondantes peuvent compléter des lignes et colonnes ;
chaque joueur reçoit de nouveaux jetons dans la limite de 21 ;
l'état du terrain produit les commandes des roues ;
les signatures élémentaires sont mises à jour ;
la Balance Map vérifie les blocages et les écarts statistiques.
...
garantir qu'une partie jouable ne soit pas immobilisée par le support ;
maintenir un dilemme entre échec et mat, conquête territoriale, gestion des
